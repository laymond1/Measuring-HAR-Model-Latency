...
import genotypes
from micro_models import NetworkHAR

sys.path.append(os.path.dirname(os.path.abspath(os.path.dirname('./data_providers/'))))
from data_providers.opportunity import OPPDataProvider
...
from data_providers.kuhar import KUHARDataProvider


parser = argparse.ArgumentParser("EA-based HARNAS Architecture Training")
...
parser.add_argument('--arch', type=str, default='WISBESTF1', help='which architecture to use')
...
if args.save == 'EXP':
	args.save = 'result_ea/{}/train-{}-{}'.format(args.dataset, args.save, time.strftime("%Y%m%d-%H%M%S"))
