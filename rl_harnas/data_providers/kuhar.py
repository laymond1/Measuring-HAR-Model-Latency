...
class KUHARDataProvider(DataProvider):
    multiplier = 4
    stem_multiplier = 3
    def __init__(self, data_path='.', save_path=None, train_batch_size=256, test_batch_size=256, valid_size=1000,
                 n_worker=0):

        self._save_path = save_path
        train_dataset, test_dataset, Xtest, Ytest, Xtrain, Ytrain = load_dataset(data_path + 'data/kuhar/')
    ...
    def save_path(self):
        if self._save_path is None:
            self._save_path = './dataset/kuhar'
        return self._save_path

    @property
    def data_url(self):
        raise ValueError('unable to download KU-HAR')
