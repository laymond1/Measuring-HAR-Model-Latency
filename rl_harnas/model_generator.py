...
def get_spec(nsc_list=None):
        """ nsc list is a list of nodes in vector form i.e. (0,0,0,0,0)"""

        # reset
        internal_state = []  # Initialise internal stored network state
        internal_state.append(NSC((0, 0, 0, 0, 0)))

        current_index = internal_state[-1].index
        # get_spec
        if nsc_list:
            nsc_list = [NSC(nsc) for nsc in nsc_list]
        else:
            nsc_list = internal_state

        if nsc_list[-1].type != 'Terminal':
            nsc_list.append(NSC((current_index + 1, 4, 0, 0, 0, 0)))
...
class ModelSpec:
    ...
    def __init__(self, layers):
        ...
        for layer in layers:
            ...
            # append layer.repr to self.encoding
            self.encoding = pd.concat([self.encoding, layer.repr], axis=0)
    ...
    def write(self, n_classes, n_channels, window_size, batch_size, classifier='Linear'):
        model = NASNet(self.encoding, n_classes=n_classes, n_channels=n_channels,
...
class NASNet(nn.Module):
    ...
    def __init__(self, architecture, n_channels=12,
                 n_classes=18, window_size=60, batch_size=512,
                 classifier='Linear'):
    ...
    def forward(self, x, h=None):
        ...
        if self.classifier == 'LSTM':
            x = x.view(batch_size, -1, self.intermediate_channels[-1])
            x, _ = self.lstm(x, h)
            x = x.view(batch_size, -1, 128)[:, -1, :]
